...
export function getGraphChangeDescription(prev: Partial<GraphState>, next: Partial<GraphState>): string {
  ...
  const modifiedNodes = nextNodes.filter(n => {
    ...
    const posChanged = Math.round(p.position.x) !== Math.round(n.position.x) || 
  ...
  if (modifiedNodes.length > 0) {
    ...
    const moved = modifiedNodes.filter(n => {
      const p = prevNodesMap.get(n.id)!
      return Math.round(p.position.x) !== Math.round(n.position.x) || 
             Math.round(p.position.y) !== Math.round(n.position.y)
    })
