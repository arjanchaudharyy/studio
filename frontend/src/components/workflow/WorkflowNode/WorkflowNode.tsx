...
export const WorkflowNode = ({ data, selected, id }: NodeProps<NodeData>) => {
  ...
  const clampHeight = (height: number) => Math.max(MIN_TEXT_HEIGHT, Math.min(MAX_TEXT_HEIGHT, height));
      ...
      nodes.map((node) =>
        node.id === id
          ? {
              ...
              data: {
                ...(node.data as any),
                ui: { ...(node.data as any).ui, size: { width: clampedWidth, height: clampedHeight } },
              },
            }
          : node,
      ),
  ...
  return (
    <div
      className={cn(
        ...
        !isEntryPoint && (effectiveStatus !== 'idle' || isTimelineActive) && [nodeStyle.bg, nodeStyle.border],
      ...
      style={{
        ...(isTextBlock
          ? { width: Math.max(MIN_TEXT_WIDTH, textSize.width ?? DEFAULT_TEXT_WIDTH), minHeight: Math.max(MIN_TEXT_HEIGHT, textSize.height ?? DEFAULT_TEXT_HEIGHT) }
          : isEntryPoint
    ...
    >
      ...
      {/* Header */}
      <div
        className={cn('px-3 py-2 border-b relative', isEntryPoint ? 'rounded-t-[1.5rem]' : 'rounded-t-lg')}
        style={{ borderColor: separatorColor || 'hsl(var(--border) / 0.5)', backgroundColor: headerBackgroundColor || undefined }}
      >
        <div className="flex items-start gap-2">
          ...
          <IconComponent className={cn('h-5 w-5 mt-0.5 flex-shrink-0 text-foreground', component.logo && 'hidden')} />
              ...
              <div className="min-w-0">
                {isEditingLabel ? (
                  ...
                  >
                    ...
                    {hasCustomLabel && (
                      <span className="text-[10px] text-muted-foreground opacity-70 truncate block">{component.name}</span>
                    )}
                  </div>
                )}
              </div>
              <div className="flex items-center gap-1">
                {isEntryPoint && (
                  <div style={{ display: 'none' }}>
                    <WebhookDetails url={workflowInvokeUrl} payload={entryPointPayload} apiKey={activeApiKey} open={showWebhookDialog} onOpenChange={setShowWebhookDialog} />
                  </div>
                )}
                {hasUnfilledRequired && !nodeData.status && <span className="text-red-500 text-xs" title="Required fields missing">!</span>}
                {StatusIcon && (!isTimelineActive || effectiveStatus !== 'running' || isPlaying) && effectiveStatus !== 'success' && (
                  <StatusIcon className={cn('h-4 w-4 flex-shrink-0', nodeStyle.iconClass, isTimelineActive && effectiveStatus === 'running' && isPlaying && 'animate-spin', isTimelineActive && effectiveStatus === 'error' && 'animate-bounce')} />
                )}
                ...
                {mode === 'design' && !isEntryPoint && (
                  <button
                    type="button"
                    className={cn('p-1 rounded hover:bg-destructive/10 text-muted-foreground/40 hover:text-destructive transition-all', isMobile ? 'opacity-100' : 'opacity-0 group-hover:opacity-100')}
                    title="Delete node"
        ...
        <div className="px-3 py-2 border-b border-border/50 bg-muted/30 space-y-2">
          {visualState.status === 'running' && (
            <Badge variant="secondary" className="text-xs bg-blue-100 text-blue-700 border border-blue-300 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700">
              {playbackMode === 'live' ? (<><Activity className="h-3 w-3 mr-1 animate-pulse" />Live</>) : isPlaying ? (<><Activity className="h-3 w-3 mr-1" />Running</>) : (<><Pause className="h-3 w-3 mr-1" />Paused</>)}
            </Badge>
          )}
          ...
          {visualState.status === 'error' && (
            <Badge
              variant="secondary"
              className={cn('text-xs bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 cursor-pointer select-none transition-all hover:ring-2 hover:ring-red-400/50 items-center gap-1', showErrorDetails && 'ring-2 ring-red-400/50')}
              onClick={() => setShowErrorDetails(!showErrorDetails)}
              title="Click to toggle error details"
            >
              <AlertCircle className="h-3 w-3" />
              <span>{visualState.lastEvent?.error?.type || 'Failed'}</span>
              <ChevronDown className={cn('h-3 w-3 transition-transform duration-200', showErrorDetails && 'rotate-180')} />
            </Badge>
          )}
          {visualState.status === 'skipped' && (
            <Badge variant="secondary" className="text-xs bg-slate-100 text-slate-600 border border-slate-300 dark:bg-slate-800/50 dark:text-slate-400 dark:border-slate-600">
              <LucideIcons.Ban className="h-3 w-3 mr-1" />Skipped
            </Badge>
          )}
          {visualState.lastMetadata?.childRunId && (
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); navigate(`/runs/${visualState.lastMetadata!.childRunId}`); }}
            >
              <ExternalLink className="h-3 w-3" />View Child Run
            </Button>
          )}
      ...
      <div className={cn('px-3 py-3 pb-4 space-y-2', isTextBlock && 'flex flex-col flex-1')}>
        {effectiveStatus === 'awaiting_input' && visualState.humanInputRequestId && (
          <div className={cn('mb-2', isEntryPoint && 'hidden')}>
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); openHumanInputDialog(visualState.humanInputRequestId!); }}
            >
              <LucideIcons.ShieldAlert className="w-3.5 h-3.5 mr-2" />Action Required
            </Button>
          </div>
        )}
        ...
        {isTextBlock && (
          trimmedTextBlockContent.length > 0 ? (
            <MarkdownView
              ...
              className={cn('w-full rounded-md border border-dashed border-muted-foreground/30 bg-muted/40 px-3 py-3 overflow-x-hidden overflow-y-auto break-words', 'prose prose-base dark:prose-invert max-w-none text-foreground', 'flex-1 min-h-0')}
              onEdit={(next) => {
                setNodes((nds) =>
                  nds.map((n) => {
                    if (n.id !== id) return n;
                    const currentConfig = (n.data as any).config || { params: {}, inputOverrides: {} };
                    return { ...n, data: { ...n.data, config: { ...currentConfig, params: { ...currentConfig.params, content: next } } } };
                  }),
                );
                markDirty();
              }}
            ...
            <div className={cn('rounded-md border border-dashed border-muted-foreground/30 bg-muted/40 px-3 py-2 text-sm text-muted-foreground leading-relaxed', 'flex-1 min-h-0')} data-testid="text-block-content">
        ...
        {isEntryPoint && (
          <div className="flex gap-3 mt-1">
            <div className="flex-[0.6] flex flex-col gap-1.5">
              <button type="button" onClick={(e) => { e.stopPropagation(); onOpenWebhooksSidebar ? onOpenWebhooksSidebar() : setShowWebhookDialog(true); }} className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-border bg-muted/60 hover:bg-muted transition-colors text-[10px] font-medium text-muted-foreground hover:text-foreground w-fit">
                <LucideIcons.Webhook className="h-3 w-3 flex-shrink-0" /><span>Webhooks</span>
              </button>
              <button type="button" onClick={(e) => { e.stopPropagation(); onOpenScheduleSidebar ? onOpenScheduleSidebar() : navigate(`/schedules?workflowId=${workflowId}`); }} className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-border bg-muted/60 hover:bg-muted transition-colors text-[10px] font-medium text-muted-foreground hover:text-foreground w-fit">
                <LucideIcons.CalendarClock className="h-3 w-3 flex-shrink-0" /><span>Schedules</span>
              </button>
              <button type="button" onClick={(e) => { e.stopPropagation(); if (mode === 'design' && nodeRef.current) { const clickEvent = new MouseEvent('click', { bubbles: true, cancelable: true, view: window }); setTimeout(() => nodeRef.current?.dispatchEvent(clickEvent), 10); } }} className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-border bg-muted/60 hover:bg-muted transition-colors text-[10px] font-medium text-muted-foreground hover:text-foreground w-fit">
                <LucideIcons.Settings className="h-3 w-3 flex-shrink-0" /><span>Inputs</span>
              </button>
            </div>
            <div className="flex-[0.4] flex flex-col justify-start">
              {effectiveOutputs.length > 0 ? (
                <div className="space-y-1.5">
                  {effectiveOutputs.map((output) => (
                    <div key={output.id} className="relative flex items-center justify-end gap-2 text-xs">
                      <div className="flex-1 text-right"><div className="text-muted-foreground font-medium">{output.label}</div></div>
                      <Handle type="source" position={Position.Right} id={output.id} className="!w-[10px] !h-[10px] !border-2 !border-green-500 !bg-green-500 !rounded-full" style={{ top: '50%', right: '-18px', transform: 'translateY(-50%)' }} />
                    </div>
                  ))}
                </div>
              ) : (
                <div className="relative flex items-center justify-end gap-2 text-xs">
                  ...
                  <Handle type="source" position={Position.Right} className="!w-[10px] !h-[10px] !border-2 !border-blue-500 !bg-blue-500 !rounded-full" style={{ top: '50%', right: '-18px', transform: 'translateY(-50%)' }} />
                </div>
              )}
            </div>
          </div>
        ...
        {/* Parameters Display */}
        {!isEntryPoint && (
          <ParametersDisplay componentParameters={componentParameters} requiredParams={requiredParams} nodeParameters={nodeData.config?.params} position="top" />
        )}
          ...
          <div className="space-y-1.5">
            {componentInputs.map((input) => {
              ...
              if (!manualValueProvided && connection) {
                ...
                if (sourceNode) {
                  const sourceComponent = getComponent((sourceNode.data as any).componentId ?? (sourceNode.data as any).componentSlug);
                  if (sourceComponent) {
              ...
              const manualDisplayVal = manualValueProvided && inputSupportsManualValue(input) && typeof manualCandidate === 'string' ? manualCandidate.trim() : '';
              let manualDisplay = manualDisplayVal;
              const isSecretInput = input.editor === 'secret' || (input.connectionType?.kind === 'primitive' && input.connectionType.name === 'secret');
              ...
              const handleClassName = cn('!w-[10px] !h-[10px] !border-2 !rounded-full', input.required ? '!bg-blue-500 !border-blue-500' : '!bg-background !border-blue-500');

              return (
                <div key={input.id} className="relative flex items-center gap-2 text-xs">
                  <Handle type="target" position={Position.Left} id={input.id} className={handleClassName} style={{ top: '50%', left: '-18px', transform: 'translateY(-50%)' }} />
                  <div className="flex-1">
                    <div className="text-muted-foreground font-medium">{input.label}</div>
                    {input.required && !sourceInfo && !manualValueProvided && <span className="text-red-500 text-[10px]">*required</span>}
                    {manualValueProvided && manualDisplay && (
                      <div className="mt-0.5">
                        <span className={cn('font-mono px-1 py-0.5 rounded text-[10px] truncate max-w-[120px] inline-flex items-center gap-1', isSecretInput ? 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50 font-semibold' : 'text-foreground bg-muted')} title={manualDisplay}>
                          {isSecretInput && <KeyRound className="h-2.5 w-2.5" />}{previewText}
                        </span>
                      </div>
                    )}
                    {manualValueProvided && !manualDisplay && <span className="text-muted-foreground text-[10px] italic">Manual value</span>}
                    {!manualValueProvided && sourceInfo && <span className="text-muted-foreground text-[10px] italic" title={`Connected to: ${sourceInfo}`}>{sourceInfo}</span>}
                  </div>
                </div>
              );
            })}
        ...
        {!isEntryPoint && (() => {
          ...
          const legacyActiveBranchId = !activatedPorts && data ? (data.approved === true ? 'approved' : data.approved === false ? 'rejected' : null) : null;
            ...
            <div className="mt-2 pt-2 border-t border-dashed border-amber-300/50 dark:border-amber-700/50">
              <div className="flex gap-2">
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <span className="text-[9px] font-medium text-amber-600/80 dark:text-amber-400/80 uppercase tracking-wider">Branches</span>
                </div>
                <div className="flex flex-col flex-1 gap-1">
                  {branchingOutputs.map((output: any) => {
                    const isActive = isNodeFinished && (activatedPorts ? activatedPorts.includes(output.id) : legacyActiveBranchId === output.id);
                    const isInactive = isNodeSkipped || (isNodeFinished && !isActive);
                    const branchColor = output.branchColor || 'amber';

                    return (
                      <div key={output.id} className="relative flex items-center justify-end gap-2 text-xs">
                        <div className={cn('flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium transition-all', 'border', !hasBranchDecision && designModeColors[branchColor], isActive && 'bg-green-50 dark:bg-green-900/30 border-green-400 dark:border-green-600 text-green-700 dark:text-green-300 ring-1 ring-green-400/50', isInactive && 'bg-slate-50/50 dark:bg-slate-900/20 border-dashed border-slate-200 dark:border-slate-800 text-slate-300 dark:text-slate-600 opacity-30 grayscale-[0.8]')}>
                          {isActive && <LucideIcons.Check className="h-2.5 w-2.5" />}
                          {isInactive && <LucideIcons.X className="h-2.5 w-2.5 text-slate-400/50" />}
                          <span>{output.label}</span>
                        </div>
                        <Handle type="source" position={Position.Right} id={output.id} className={cn('!w-[10px] !h-[10px] !border-2 !rounded-full', !hasBranchDecision && handleDesignColors[branchColor], isActive && '!border-green-500 !bg-green-500', isInactive && '!border-slate-300 !bg-slate-200 dark:!bg-slate-800 opacity-30')} style={{ top: '50%', right: '-18px', transform: 'translateY(-50%)' }} />
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="pt-2 border-t border-border/50">
            {visualState.lastEvent && (
              <div className="text-xs text-muted-foreground mt-2">
                <div className="font-medium">Last: {visualState.lastEvent.type.replace('_', ' ')}</div>
                {visualState.lastEvent.message && <div className="truncate mt-1" title={visualState.lastEvent.message}>{visualState.lastEvent.message}</div>}
              </div>
            )}
          </div>
        )}
        ...
        {!isTimelineActive && nodeData.status === 'success' && nodeData.executionTime && (
          <div className="pt-2 border-t border-border">
            <Badge variant="secondary" className="text-xs bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300">✓ {nodeData.executionTime}ms</Badge>
          </div>
        )}
        {!isTimelineActive && nodeData.status === 'error' && nodeData.error && (
          <div className="pt-2 border-t border-red-200">
            <Badge variant="secondary" className="text-xs bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 truncate max-w-full" title={nodeData.error}>✗ {nodeData.error}</Badge>
          </div>
        )}
      </div>
    </div>
