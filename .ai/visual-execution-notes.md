...
## 2025-11-16 · Workflow Inspector + Worker Follow-ups

- **Execution Inspector stall:** Loading `/workflows/:id` fired 5–6 concurrent `GET /api/v1/workflows/runs?limit=50` requests from `WorkflowBuilder` and `RunSelector`, each of which forced the backend to re-describe every Temporal workflow, so the “Inspect executions” tab sat on “Loading…” for ~45 s. Guarded `useExecutionTimelineStore.loadRuns` with `isLoadingRuns`/`runsLoadedAt` and only force-refresh after starting a run, but the real fix will be to centralise data-fetching (React Query or a dedicated Zustand slice) so components don’t trigger duplicate loads.
- **httpx output mismatch:** DNSX handed 326 domains to httpx but the UI showed zero results because the docker runner returned a JSON string (with `{results:[...]}`) and the component only parsed NDJSON lines. Updated `worker/src/components/security/httpx.ts` to `JSON.parse` string outputs before validation and re-ran the `ENABLE_HTTPX_COMPONENT_TESTS=true` suite so `outputSummary.resultCount` finally matches the raw data surfaced in logs.
- **Temporal worker offline:** `pm2 logs shipsec-worker` still reports `tonic::transport::Error … ConnectError("tcp connect error", 127.0.0.1:7233)` and the stack traces come from `/studio-root/studio-text/worker/...`, meaning the pm2 process was launched from an older worktree and never re-pointed to this repo. Until the worker connects to the dockerised Temporal namespace (`shipsec-dev` on port 7233), new workflow runs remain stuck in `RUNNING` with no httpx/DNSX progress. Next steps: ensure pm2 uses `studio-main/worker` binaries, verify `.env` points to `TEMPORAL_ADDRESS=127.0.0.1:7233`, and confirm the docker network allows host connections.
