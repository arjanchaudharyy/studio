...
|---------|--------|------------|
| **Source** | Upstream nodes or Manual Overrides | User configuration in Sidebar |
| **Schema** | Defined via `inputs()` and `port()` | Defined via `parameters()` and `param()` |
| **UI** | Visible in the "Inputs" section | Visible in the "Parameters" section |
| **Mapping** | Can be connected to other nodes | Static values only |

### Defining Ports

Use the `port()` helper to add metadata to your Zod schemas:

```typescript
const inputSchema = inputs({
  domain: port(z.string(), {
    label: 'Target Domain',
    description: 'The root domain to enumerate.',
    connectionType: { kind: 'primitive', name: 'text' },
  }),
  apiKey: port(z.string(), {
    label: 'API Key',
    editor: 'secret', // Uses masked input in UI
...
Use the `param()` helper for static settings:

```typescript
const parameterSchema = parameters({
  timeout: param(z.number().default(30), {
    label: 'Timeout (s)',
    editor: 'number',
    min: 1,
    max: 300,
